...
class FrameworkDetector(BaseDetector):
    def detect(self) -> List[Tuple[str, str]]:
        ...
        for framework_name, patterns in self.signatures.items():
            if self._detect_single(framework_name, patterns):
                version = self._extract_version(framework_name, patterns)
                detected_frameworks.append((framework_name, version))

        return detected_frameworks

    def _detect_single(self, framework_name: str, patterns: Dict[str, Any]) -> bool:
    ...
    def _extract_version(self, framework_name: str, patterns: Dict[str, Any]) -> str:
